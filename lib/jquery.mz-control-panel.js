...
MZControlPanel.prototype.create = async function(mz700js, mzBeep) {
    ...
    this._emulationPanel
        ...
        .append($("<input/>").EmuSpeedControl(
            "create", this._mz700js, MZ700.DEFAULT_TIMER_INTERVAL,
            interval => this._mz700js.setExecutionParameter(interval)))
    ...
    const onkey = (event, state) => {
        if(this._acceptKey) {
            event.stopPropagation();
            this.updateKeyStates(event, state);
            return false;
        }
    };
...
MZControlPanel.prototype.createSoftwareKeyboard = function() {
    this._keyboard.mz700keyboard("create", {
        onStateChange: (strobe, bit, state) => {
            this._mz700js.setKeyState(strobe, bit, state);
        }
    });
...
MZControlPanel.prototype.createDataRecorderControl = function() {
        ...
        el.addEventListener('drop', event => {
            ...
            if(files.length > 0) {
                ...
                reader.onload = async () => {
                    ...
                    await this._mz700js.setCassetteTape(tape_data);
                };
    ...
    this._mz700js.subscribe("onStartDataRecorder", () => {
        btnCmtRec.prop("disabled", true);
        btnCmtEject.prop("disabled", true);
        btnCmtStop.prop("disabled", false);
    });

    this._mz700js.subscribe("onStopDataRecorder", async ()=>{
        await this.updateCmtSlot();
        btnCmtRec.prop("disabled", false);
        btnCmtEject.prop("disabled", false);
        btnCmtStop.prop("disabled", true);
    });
};
...
// Handle Key events
MZControlPanel.prototype.updateKeyStates = function(event, state) {
    const code = event.keyCode;
    const matrix = MZ700KeyMatrix.Code2Key[code];
    if(!(code in this._keystates) || this._keystates[code] != state) {
        this._keystates[code] = state;
        if(matrix != null) {
            this._mz700js.setKeyState(
                matrix.strobe, matrix.bit, state);
        }
    }
    if(matrix != null) {
        this._keyboard.mz700keyboard("setState",
            matrix.strobe, matrix.bit, state);
    }
};
