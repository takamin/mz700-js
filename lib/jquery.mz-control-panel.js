...
MZControlPanel.prototype.create = async function(mz700js, mzBeep) {
    ...
    this._emuSpeedControl = $("<span/>").EmuSpeedControl(
        "create", MZ700.DEFAULT_TIMER_INTERVAL);
    this._emuSpeedControl.get(0).addEventListener("timerIntervalChange", event => {
        const routeEvent = new Event("EmuTimerIntervalChange");
        routeEvent.timerInterval = event.timerInterval;
...
MZControlPanel.prototype.emuTimerInterval = function(timerInterval) {
    this._emuSpeedControl.EmuSpeedControl("timerInterval", timerInterval);
};
