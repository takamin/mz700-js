...
EmuSpeedControl.prototype.create = function(mz700js, baseClock) {
    ...
    mz700js.subscribe("onNotifyClockFreq", tCyclePerSec => {
        this.actualClock(tCyclePerSec);
    });

    // Handling events about the emulation speed control slider
    if(Cookies.get("clockFactor")) {
        const clockFactor = parseFloat(Cookies.get("clockFactor"));
        mz700js.setClockFactor(clockFactor);
        this.clockFactor(clockFactor);
    } else {
        mz700js.getClockFactor().then(
            clockFactor => this.clockFactor(clockFactor)
        );
    }
    ...
    mz700js.subscribe("onClockFactorUpdate", clockFactor => {
        this.clockFactor(clockFactor);
        Cookies.set("clockFactor", clockFactor, cookieOpt);
    });
